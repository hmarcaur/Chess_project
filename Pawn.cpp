...
bool Pawn::isValidMove(const Position &start, const Position &end, const Board* board) const{
	...
	*/
	...
	if(firstMove && !getIsBlack() && board->emptySquare(end.get_x(), end.get_y()) \
	&& start.get_x() == end.get_x() && end.get_y() == start.get_y() + 2 \
	...
	if(!getIsBlack() && start.get_y() == 4 && start.get_distX(end) == 1 && end.get_y() == start.get_y() + 1 \
		&& !board->emptySquare(end.get_x(), start.get_y()) && board->getPiece(p2)->toString() == "Pa"){
		return board->emptySquare(end.get_x(), end.get_y()) && board->getLastMove().start == Position(end.get_x(), start.get_y() + 2) \

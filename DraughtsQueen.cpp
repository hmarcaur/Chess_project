...
DraughtsQueen::DraughtsQueen(bool isBlack): Piece(isBlack, "queen"){
	/*
	Class constructor 'DraughtsQueen'.
	*/
}

bool DraughtsQueen::isValidMove(const Position &start, const Position &end, const Board* board) const{
	/*
...
Position DraughtsQueen::isValidCatch(const Position &start, const Position &end, const Board* board, const Position &initialPosition) const{
	/*
...
bool DraughtsQueen::isValidTrip(const Position &start, const Position &end, const Board* board) const{
	/*
	Returns 'true' if the 'DraughtsQueen' in position 'start' can move to 'end' without catching or encountering any piece.
	*/
  int dy = start.get_distY(end);
	if(dy == start.get_distX(end)){ // Same diagonal
    int x;
...
unordered_set<Position, PositionHash> DraughtsQueen::getValidMoves(const Position &start, Board* board) const{
	/*
	Method to get all the valid movements of a 'DraughtsQueen' piece from 'start'.
	*/
  ...
  if(board->getBestMoves(getIsBlack()).size() == 0){ // First time
    ...
    if(it->begin()->start == start) possiblePositions.insert(it->end()->end);
    else; // pass
  }

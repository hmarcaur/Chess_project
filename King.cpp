...
bool King::isValidMove(const Position &start, const Position &end, const Board* board) const{
	...
	*/
	if(start.get_distX(end) <= 1 && start.get_distY(end) <= 1) return true;
	else if(start.get_y() == end.get_y() && start.get_distX(end)){ // Castling
		...
		for(int d = 0; d <= 2; d++){ // The king moves from two squares
			for(auto it = positions.begin(); it != positions.end(); it++){
					if(board->getPiece(*it)->isValidMove(*it, Position(min_x + d, start.get_y()), board)) \
					return false; // The king is in check
				}
			...
			if(d > 0 && !board->emptySquare(min_x + d, start.get_y())) return false; // Not empty squares
		}
		// The king is not in check on its moving squares and there are empties.
